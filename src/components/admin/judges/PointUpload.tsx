...
function PointUpload(props : Props) {
    ...
    const TotalMarkSetter = ( i : number, value : string , chestNo : string)=>{
      ...
      let inputOne = (document.getElementById(`input-${0}-of-${chestNo}`) as HTMLInputElement)?.value || '0'
      let inputTwo = (document.getElementById(`input-${1}-of-${chestNo}`) as HTMLInputElement)?.value || '0'
      let inputThree = (document.getElementById(`input-${2}-of-${chestNo}`) as HTMLInputElement)?.value || '0'
      ...
      (document.getElementById(`total-${chestNo}`) as any).innerHTML =( (parseFloat(inputOne as string) + parseFloat(inputTwo as string) + parseFloat(inputThree as string)) / pointsCount ).toFixed(2)
  ...
  return (
    <>
    {/* mobile only */}
    <div className='fixed h-screen w-screen bg-white  md:bg-white md:opacity-30 top-0 left-5 md:left-0'>
      ...
      </div>
{
     props.Programme?.candidateProgramme?.map((item , index)=>{
        return(
          <div className="flex flex-col gap-3 h-[70%] overflow-y-auto">
          {/* list1 */}
      ...
      <div className="h-16 flex items-center justify-between  px-5">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-2" onClick={()=>{
            ...
            <button className={`h-4 w-4  ${pointsCount == 3 ? 'bg-slate-600' :'bg-green-500' } rounded`}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
          ...
          <div className="flex items-center gap-2"  onClick={()=>{
            ...
            <button className={`h-4 w-4  ${pointsCount == 1 ? 'bg-slate-600' :'bg-red-500' } rounded`}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-[90%] w-[90%] fill-white"
                viewBox="0 -960 960 960"
                    ...
                    <div className="flex gap-5 bg-accent h-14 w-2/3 rounded-xl px-5 items-center text-[10px] justify-between">
                      ...
                      {[...Array(pointsCount)].map((_, i) => { 
                        return(
                            ...
                            TotalMarkSetter(i , e.target.value , item.candidate?.chestNO as string )
                            }}
                          />
                        )
                      })}
                      ...
                      <div className="flex gap-5 items-center">
                        <p>total</p>
                        <div id={`total-${item.candidate?.chestNO}`} className="bg-white h-10 w-12 rounded-lg justify-center flex items-center text-lg font-semibold text-primary">
                          0
                        </div>
                      </div>
                    </div>
